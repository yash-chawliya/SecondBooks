...
const AuthComponent = () => {
    ...
    const { login, itemToAddAfterLogin, setName, isLoggedIn } = useAuth(); // Get isLoggedIn status

    // --- NEW: Redirect if already logged in ---
    useEffect(() => {
        // If the user is already logged in, they shouldn't be on this page.
        // Redirect them to the homepage.
        if (isLoggedIn) {
            navigate('/', { replace: true });
        }
    }, [isLoggedIn, navigate]); // This effect runs when isLoggedIn status changes
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (isLoginMode) {
                ...
                login(userId); // This will update the global state
    ...
    return (
        <form onSubmit={handleSubmit} className='m-auto w-full max-w-sm border text-center mt-16 p-4 rounded-lg shadow-lg'>
            <Toaster position="top-right" />
            <div className='font-bold text-2xl my-5'>{isLoginMode ? 'LOGIN' : 'CREATE AN ACCOUNT'}</div>

            {/* --- Conditional Fields for Sign Up --- */}
            {!isLoginMode && (
                <>
                    <div className="flex items-center gap-3 rounded-md border border-gray-300 px-4 py-2 bg-white shadow-sm w-full m-auto mb-3 focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-500">
            ...
            {/* Email Input */}
            <div className="flex items-center gap-3 rounded-md border border-gray-300 px-4 py-2 bg-white shadow-sm w-full m-auto">
            ...
            {/* Password Input */}
            <div className="flex items-center gap-3 rounded-md border border-gray-300 px-4 py-2 bg-white shadow-sm w-full m-auto mt-3">
